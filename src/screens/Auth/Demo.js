...
const StackNav = props => {
  ...
  console.log('vbsvd', AuthReducer?.signinResponse?.data?.[0]?.role_name);
  const MyTransition = {
  ...
  //         UpdateProfile: SetUpProfile,
  //       };
// console.log('jyfjytftuhjfhy', AuthReducer?.signinResponse?.data?.[0]?.role_name);
  const Screens =
    // AuthReducer?.token == null
    //   ? {
    //       OnboardingScreen: OnboardingScreen,
    ...
    //     }
    //   : 
      AuthReducer?.signinResponse?.data?.[0]?.role_name == 'USER'
      ? {
          Dashboard: Dashboard,
          BottomTabNav: BottomTabNav,
          AskAQuestion: AskAQuestion,
